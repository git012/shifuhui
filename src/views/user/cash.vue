        ...
        <Card>
            ...
            <div class="doBox">
                ...
                <div class="ordler-list" v-if="doType=='list'">
                    ...
                    <div class="search-box" style="width: 100%;line-height: 50px;height: 50px;text-align: left"><span>申请提现总额（元）：{{Totals.sum_total}}</span><span style="padding: 0 0px 0 30px">待审核总计（元）：{{Totals.wait}}</span><span style="padding: 0 30px">提现成功总计(元)：{{Totals.success}}</span><span>提现失败总计（元）：{{Totals.fail}}</span></div>
        ...
        methods: {
            ...
            downloadlist(){
                let url=Config.apiRootPath+Config.api.user.user_cash.dw;
                    location.href=url+"&ssid="+Cookies.get('ssid')+"&startDate="+this.searchData.startDate+"&endDate="+this.searchData.endDate+"&type=1";
            },
            getList () {
                let postData={
                ...
                $.ajax({
                    url: Config.apiRootPath+Config.api.user.user_cash.list,
                    type: 'POST',
                    dataType: 'json',
                    data: postData
                })
                        ...
                        if(!!data){
                            if(data.code==0){
                                ...
                                if(data.data.sum_list==undefined){
                                    ...
                                    this.Totals.fail=data.data.sum_list.reject_total==null?"0.00":(data.data.sum_list.reject_total)/100;
                                    this.Totals.wait=(data.data.sum_list.unreviewed_total)/100;
                                    this.Totals.success=(data.data.sum_list.consent_total)/100;
                                    this.Totals.sum_total=(data.data.sum_list.sum_total)/100;
                                }

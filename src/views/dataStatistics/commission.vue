        ...
        <Card>
         ...
         <DatePicker v-bind:type="typeEnd" v-model="searchData.endDate" placeholder="请选择结束时间" style="width: 200px"></DatePicker>
            ...
            <Row type="flex" justify="start" class="code-row-bg">
                <Col span="4">提现手续费(万)：<span class="num" style="color:red;">{{count.successTotalAmount}}</span></Col>
                <Col span="6">代理分红转入预存款手续费(万)：<span class="num" style="color:#c33531;">{{count.fail}}</span></Col>
                <Col span="6">消费增值积分转入预存款手续费(万)：<span class="num" style="color:#2e4553;">{{count.waitTotalAmount}}</span></Col>
            ...
            drawPie(id){
                ...
                this.charts.setOption({
                    ...
                    legend: {
                        ...
                        data:['提现手续费','代理分红转入预存款手续费','消费增值积分转入预存款手续费']
                    },
                    toolbox: {
                        show : true,
                        feature : {
...
//下载
            ...
            downloadlist(){
                ...
                let url=Config.apiRootPath+Config.api.dataStatistics.commission.dw;
                if(this.searchData.startDate==""&&this.searchData.endDate==""){
            ...
            getList(){
                ...
                $.ajax({
                    url:Config.apiRootPath+Config.api.dataStatistics.commission.list,
                    type: 'POST',
                    dataType: 'json',
                    data: postData
                })
                    .done((data)=>{
                        ...
                        if(!!data){
                            if(data.code==0){
                                this.count.successTotalAmount=(Number(data.data.sum.charge!=null?data.data.sum.charge:'0.00')/10000/100).toFixed(6);
                                // this.count.type_1_sum=Number(data.data.sum.type_1_sum!=null?data.data.sum.type_1_sum:0)/10000;
                                this.count.fail=(Number(data.data.sum.charge_2!=null?data.data.sum.charge_2:'0.00')/10000/100).toFixed(6);
                                this.count.waitTotalAmount=(Number(data.data.sum.charge_3!=null?data.data.sum.charge_3:'0.00')/10000/100).toFixed(6);
                                ...
                                if(data.data.list==undefined){
                                    ...
                                    for(var i=0;i<data.data.list.length;i++){
                                        ...
                                        this.type_1_sum.push((Number(data.data.list[i].charge)/100/10000).toFixed(6));
                                        this.type_2_sum.push((Number(data.data.list[i].charge_2)/100/10000).toFixed(6));
                                        this.type_3_sum.push((Number(data.data.list[i].charge_3)/100/10000).toFixed(6));
